...
##
  longdf <- NULL
  no_runs <- NULL
  c_5pros <- c("bi_mst_500","bi_ind_750","bi_airport_5k","bi_ndvi_300") #change into 4ep
  repeat_loop <- 100 #still 100 runs
  set.seed(1)
  t_sub_n <- 80  #only use 40 and 80
  ########
  #set end
  #start
  ########
  ...
  c4_3 <- list(c(1,2,3),c(1,2,4),c(1,3,4), c(2,3,4)       )
  for (j in c4_3){
    j1 <- j[1]
    j2 <- j[2]
    j3 <- j[3]
...
#save it
write.table(longdf,paste0("4_r_output/sims_result_",t_sub_n,".csv"),sep=",",row.names = F)
