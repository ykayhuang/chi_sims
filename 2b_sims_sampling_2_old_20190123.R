...
#use aggregate to gruop the cond_loop,there will be (runs)*(loops tried) obs


#df4_mean <- aggregate(df3[,2:22],by=list(df3$cond_loop),mean,na.rm=T)
#sp4 <- df4_mean[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
#df4_mean <- cbind(sp4,df4_mean) %>% as.data.frame()
df4_mean <- aggregate(df3[,2:22],by=list(df3$cond_loop),mean,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_p50 <- aggregate(df3[,2:22],by=list(df3$cond_loop),median,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_p75 <- aggregate(df3[,2:22],by=list(df3$cond_loop),quantile,probs=.75,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_p25 <- aggregate(df3[,2:22],by=list(df3$cond_loop),quantile,probs=.25,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_min <- aggregate(df3[,2:22],by=list(df3$cond_loop),min,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_max <- aggregate(df3[,2:22],by=list(df3$cond_loop),max,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_var <- aggregate(df3[,2:22],by=list(df3$cond_loop),var,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
df4_sd <- aggregate(df3[,2:22],by=list(df3$cond_loop),sd,na.rm=T) %>% 
  cbind(.[,1] %>% as.data.frame %>%`colnames<-`("x") %>% separate(.,x,c("cond","loop.count"), sep = "-")
        ,.) %>% as.data.frame() %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))
 

df4_iqr <- cbind(df4_p75[,1:3],(df4_p75[,4:13]-df4_p25[,4:13]),df4_p75[,14:24]) %>% mutate(pros=str_count(cond,"bi")) %>% mutate(grp=paste(pros,"ep:",cond))

#sims_by_cond_mean <- aggregate(df4_mean[,4:24],by=list(df4_mean$cond),mean,na.rm=T)

sim_iqr_by_cond_mean <- aggregate(df4_iqr[,4:13],by=list(df4_iqr$cond),mean,na.rm=T)
sim_iqr_by_cond_mean <- aggregate(df4_iqr[,4:13],by=list(df4_iqr$cond),mean,na.rm=T)
sim_iqr_by_cond_sd <- aggregate(df4_iqr[,4:13],by=list(df4_iqr$cond),sd,na.rm=T)
