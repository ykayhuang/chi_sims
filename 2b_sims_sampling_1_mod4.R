...
#20190126 log: this is for the fixing the uneveing prob
...
fun_sims_with_t_sub_n <- function(t_sub_n){
  ...
  c4_2 <- list(c(1,2),c(1,3),c(1,4),c(2,3),c(2,4),c(3,4)       )
  for (j in c4_2){
    j1 <- j[1]
    j2 <- j[2]
...
for(d in c(5:25)){
  ...
  write.table(longdf,paste0("4_r_output/sims_fixed_bad_all/sims_result_",t_sub_n,".csv"),sep=",",row.names = F)
  print(t_sub_n)
}