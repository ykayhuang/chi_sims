...
for (ldf_m_s_c in c(2,5,8,11)){
  ...
  p2 <- p2+theme_bw(base_size = 26)+ theme(legend.position="right") + expand_limits(y =0)
...
for (ldf_m_s_c in c(2,5,8,11)){
  ...
  p4 <- p4 +scale_color_manual(values=c("black", "gray32","gray32","gray32","gray32",
                                       "gray58","gray58","gray58","gray58","gray58","gray58")) 
  p4 <- p4 +theme_bw(base_size = 26)+ theme(legend.position="right") + expand_limits(y =0)
...
ddr6 <- subset(ldf_mis_summary,cond%in%c("bi_airport_5k","all random")) ##for comparing fixed and all random
...
for (x in 5:25){
  for (y in 1:11){
    thisn <- 4*x
    z7 <- ldf_mis_count[ldf_mis_count$thisn==thisn,] %>% subset(.,cond==as.character(ldf_mis_summary[y,1]))
    z8 <- ldf_mis_summary[ldf_mis_summary$thisn==thisn,]
